...
namespace CCloud
{
    class Program
    {
        ...
        {
            Console.WriteLine($"{nameof(Consume)} started");
            ...
            using (var consumer = new ConsumerBuilder<string, string>(consumerConfig).Build())
            {
                ...
                try
                {
                    while (true)
                    {
                        ...
                        byte[] traceParentByte = cr.Message.Headers[0].GetValueBytes();
                        var traceparent = Encoding.ASCII.GetString(traceParentByte);
                        Console.WriteLine($"Consumed record with TraceParent: {traceparent}");
                        var distributedTracingData = DistributedTracingData.TryDeserializeFromString(traceparent);
                        var transaction2 = Agent.Tracer.StartTransaction("consumer", "TestTransaction", distributedTracingData);
                        transaction2.End();
                    }
                }
